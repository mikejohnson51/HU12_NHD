...
plan <- drake_plan(
  ...
  temp_dir = "temp/",
  nhdhr_dir = download_nhdhr(nhd_dir, hu02, nhdhr_bucket, nhdhr_file_list),
  nhdhr = get_nhdplushr(nhdhr_dir, hu02)
  # wbd_gdb = download_wbd(wbd_dir, wbd_zip_file, wbd_gdb_file, wbd_url),
  # fixes = get_fixes(wbd_version),
  # net = get_net(nhdhr, prj),
  ...
  # exclude = get_exclusions(wbd_gdb),
